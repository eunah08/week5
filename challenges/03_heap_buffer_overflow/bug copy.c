...
static void list_ensure(IntList *l, size_t need)
{
    ...
    // 새 용량인 newcap이 아니라 기존 용량인 l->cap으로 realloc을 호출함
    // newcap은 커졌지만 실제로 할당하는 메모리는 기존 용량만큼이라 실제 버퍼가 한 세대 뒤처짐
    if (!p) { perror("realloc"); free(l->data); exit(1); }

    l->data = p;

    l->cap  = newcap;
    // l->cap에는 새 용량인 newcap을 저장하지만 realloc은 이전 용량으로 실행되어
    //   논리적인 용량과 실제 할당된 버퍼의 크기가 달라짐
}
...
static void list_push(IntList *l, int x)
{
    ...
    l->data[l->len++] = x;
    // l->cap을 기준으로 공간이 있다고 생각하고 데이터를 저장하지만
    //   실제 버퍼는 더 작아서 버퍼의 범위를 벗어나 데이터를 쓰게 됨
}
